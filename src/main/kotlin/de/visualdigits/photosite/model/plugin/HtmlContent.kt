...
open class HtmlContent(
    ...
    protected fun obfuscateText(html: String): String {
        ...
        while (matcher.find()) {
            h = h.replace(matcher.group(0), obfuscate(matcher.group(1), ObfuscateType.TEXT))
        }
    ...
    protected fun obfuscateEmail(html: String): String {
        ...
        while (matcher.find()) {
            h = h.replace(matcher.group(0), obfuscate(matcher.group(1), ObfuscateType.EMAIL))
        }

        return h
    }

    private fun obfuscate(text: String, obfuscateType: ObfuscateType): String {
